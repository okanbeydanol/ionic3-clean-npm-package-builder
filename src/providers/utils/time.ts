...
export class CoreTimeUtilsProvider {
  private SECONDS_YEAR = 31536000;
  private SECONDS_WEEK = 604800;
  private SECONDS_DAY = 86400;
  private SECONDS_HOUR = 3600;
  private SECONDS_MINUTE = 60;
  private WIFI_DOWNLOAD_THRESHOLD = 104857600; // 100MB.
  private DOWNLOAD_THRESHOLD = 10485760; // 10MB.
  private MINIMUM_FREE_SPACE = 10485760; // 10MB.
  private IOS_FREE_SPACE_THRESHOLD = 524288000; // 500MB.
  ...
  formatTime(seconds: number): string {
    ...
    years = Math.floor(totalSecs / this.SECONDS_YEAR);
    remainder = totalSecs - (years * this.SECONDS_YEAR);
    days = Math.floor(remainder / this.SECONDS_DAY);

    remainder = totalSecs - (days * this.SECONDS_DAY);

    hours = Math.floor(remainder / this.SECONDS_HOUR);
    remainder = remainder - (hours * this.SECONDS_HOUR);

    mins = Math.floor(remainder / this.SECONDS_MINUTE);
    secs = remainder - (mins * this.SECONDS_MINUTE);
