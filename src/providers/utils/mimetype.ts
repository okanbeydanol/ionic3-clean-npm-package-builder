...
export class CoreMimetypeUtilsProvider {
    ...
    constructor(http: HttpClient, logger: CoreLoggerProvider,
        private textUtils: CoreTextUtilsProvider, private coreFile: CoreFileProvider) {
    ...
    getEmbeddedHtml(file: any, path?: string): string {
        ...
        if (this.canBeEmbedded(ext)) {
            ...
            path = this.coreFile.convertFileSrc(path || file.fileurl || file.url || (file.toURL && file.toURL()));
